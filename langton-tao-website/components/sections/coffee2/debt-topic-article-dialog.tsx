...
import type { DebtTopicArticleContent } from '@/lib/content/debt-topic-articles'

type DebtTopicArticleDialogProps = {
  article: DebtTopicArticleContent | null
  onClose: () => void
}

export function DebtTopicArticleDialog({
  article,
  onClose,
}: DebtTopicArticleDialogProps) {
  return (
    <Dialog
      open={article !== null}
      onOpenChange={(open) => {
      ...
      {article ? (
        <DialogContent className="debt-article-dialog inset-0 m-auto flex h-[min(90vh,52rem)] w-[calc(100%-2rem)] max-w-3xl translate-x-0 translate-y-0 flex-col overflow-hidden p-0">
          ...
          <div className="debt-article-dialog__body min-h-0 flex-1 overflow-y-auto px-5 py-5">
            ...
            <div className="debt-article-dialog__blocks mt-5 space-y-5">
